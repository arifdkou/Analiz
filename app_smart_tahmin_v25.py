...
# =========================
# 🔢 Fibo Swing Seçimi UI
# =========================
fibo_mode = st.selectbox(
    "Fibo swing seçimi",
    [
        "Fibo kapalı",
        "Son 60 bar",
        "Son 120 bar",
        "Son 240 bar",
        "Son 3 ay (otomatik tarih)"
    ]
)

show_fibo = fibo_mode != "Fibo kapalı"
...
if valid_buy.empty or valid_sell.empty or ohlc_main is None:
    ...
    ax_lines.plot([], [], color="black", linewidth=1.1, label="Fiyat (mum + fitil)")
    ...
    # 🔎 Bugünkü fiyat (Fibo tabloda da kullanacağız)
    today_x = ohlc_last.index[-1]
    today_y = ohlc_last["Close"].iloc[-1]
    close_price = float(today_y)

    # -------------------------------
    # 🔢 Fibonacci seviyeleri + TABLO
    # -------------------------------
    fibo_table = []  # tablo için liste

    if show_fibo and len(ohlc_main) > 0:
        # Fibo için kullanılacak veri penceresini seç
        if fibo_mode == "Son 60 bar":
            ohlc_fibo = ohlc_main.tail(60)
        elif fibo_mode == "Son 120 bar":
            ohlc_fibo = ohlc_main.tail(120)
        elif fibo_mode == "Son 240 bar":
            ohlc_fibo = ohlc_main.tail(240)
        elif fibo_mode == "Son 3 ay (otomatik tarih)":
            last_date = ohlc_main.index.max()
            start_date = last_date - pd.Timedelta(days=90)
            ohlc_fibo = ohlc_main[ohlc_main.index >= start_date]
        else:
            ohlc_fibo = pd.DataFrame()

        if not ohlc_fibo.empty:
            swing_low = ohlc_fibo["Low"].min()
            swing_high = ohlc_fibo["High"].max()

            fib_ratios = [
                0.00, 0.236, 0.382, 0.5, 0.618, 0.786,
                1.00, 1.272, 1.618, 2.0, 2.618
            ]

            # Her fibo seviyesi için farklı renk
            fib_colors = [
                "#FF0000",  # 0.00
                "#FF7F00",  # 0.236
                "#FFFF00",  # 0.382
            ...
            first_fib = True
            for r, col in zip(fib_ratios, fib_colors):

                # Long / Short'a göre seviye hesabı
                if position_side == "Long":
                    level = swing_low + (swing_high - swing_low) * r
                else:  # Short için ters yorum
                    level = swing_high - (swing_high - swing_low) * r

                # Grafiğe Fibo çizgisi
                ax_lines.axhline(
                    level,
                    linestyle="--",
                    linewidth=0.9,
                ...
                # 🔹 Destek / Direnç tipi
                # 0.0–0.5 arası: "Destek", üstü: "Direnç"
                tip = "Destek" if r <= 0.5 else "Direnç"

                # 🔹 Fiyata göre yüzde fark (pozitif: seviye fiyatın üstünde)
                if close_price != 0:
                    pct_diff = (level - close_price) / close_price * 100.0
                else:
                    pct_diff = 0.0
    ...
    # 📊 Fibonacci Tablosu
    if show_fibo and len(fibo_table) > 0:
        fibo_df = pd.DataFrame(
            fibo_table,
            columns=["Fibo Oranı", "Seviye", "Tip (Destek/Direnç)", "Fiyata Göre % Fark"]
        )
        st.subheader("📊 Fibonacci Seviyeleri Tablosu")
        st.dataframe(fibo_df)
...
if cycle_prices is None or cycle_curve is None:
    ...
    # Dip / tepe bantları
    ax_cycle.axhline(0.2, linestyle="--", alpha=0.7, color="green", label="Dip bölgesi (~0–2. hafta)")
    ax_cycle.axhline(0.8, linestyle="--", alpha=0.7, color="red",label="Tepe bölgesi (~10–12. hafta)")
...
if smi_series.dropna().empty:
    ...
    ax_smi.plot(smi_series.index, smi_series.values, linewidth=0.8,label="SMI (20g rolling)")
    ax_smi.plot(
        smi_series.index,
        smi_series.rolling(20).mean().values,linewidth=0.8,
...
ax.axhline(entry_price, linestyle="-", linewidth=1.1, color= "yellow", label="Entry")
ax.axhline(stop_price, linestyle="--", linewidth=1.1, color= "red", label="Stop")
ax.axhline(tp1_price, linestyle="-.", linewidth=1.1, color= "green",label="TP1")
ax.axhline(tp2_price, linestyle=":", linewidth=1.1, color= "green",label="TP2")
...
if show_fibo and ohlc_main is not None and not ohlc_main.empty:
    ...
    if not recent_ohlc.empty:
        ...
        # Her Fibo seviyesi için renk listesi
        fib_colors = [
            "#FF0000",  # 0.00   kırmızı
            "#FF7F00",  # 0.236  turuncu
            "#FFFF00",  # 0.382  sarı
            "#00FF00",  # 0.5    yeşil
            "#00FFFF",  # 0.618  cyan
            "#0000FF",  # 0.786  mavi
            "#8A2BE2",  # 1.00   mor
            "#FF1493",  # 1.272  pembe
            "#00BFFF",  # 1.618  açık mavi
            "#32CD32",  # 2.0    lime
            "#A52A2A",  # 2.618  kahverengi
        ]
